...
function fail(message: string): never {
  console.error(`\nTROTW 2.2.301 validation failed: ${message}`)
  process.exit(1)
}
...
if (packageJson.version !== '2.2.301') fail('package.json is not version 2.2.301')
...
for (const expected of ['TRotW', '/images/wardens-sigil-64.png', 'Play', 'Shape', 'Read', 'Rodney', 'Support', 'https://ko-fi.com/dodoink', "href: '/play'", "href: '/shape'", 'href="/rodney"', 'sticky top-0', 'NavDiamond', 'FullscreenToggle']) {
  if (!trotwoodHeader.includes(expected)) fail(`TRotW header is missing 2.2.301 behavior: ${expected}`)
}
...
console.log('TROTW 2.2.301 release validation passed.')
